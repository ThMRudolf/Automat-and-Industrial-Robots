...
1. ( \theta*n ) - Angle between the axes ( x*{n-1} ) and ( x_n ), rotated about ( z\_{n-1} ). 2. ( d_n ) - Distance between the origins ( O_n ) and ( O\_{n-1} ) along the ( z\_{n-1} ) axis. 3. ( a_n ) - Perpendicular distance between the ( z\_{n-1} ) and ( z_n ) axes. 4. ( \alpha*n ) - Angle between the ( z*{n-1} ) and ( z_n ) axes, rotated about ( x_n ).

**Image: insert image of Denavit-Hartenberg parameters with graphical representation of the robot's joints**

------------------------------------------------------------------------

### Mathematical Model of a 6-DOF Robot
...
### Inverse Kinematics Ambiguities
...
### Denavit-Hartenberg Transformation Exercise
