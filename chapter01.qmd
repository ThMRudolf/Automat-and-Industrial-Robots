---
title: "Introduction to Automation and Process Control"
author: "Thomas M. Rudolf"
format: html
editor: visual
---

## Definition of Automation

Replacement of the human operator by an artificial operator, in the execution of a previously programmed physical or mental task. \## Types of control schemes One has to control the system in order to autmate it.

There are basically two types of control: Open Loop and Closed Loop

The open loop has the following structure. It receives a command or reference signal. The controller calulates an input signal for the actuator. The output of the actuatar starts manipulating the system. Since the controler does not receives any feedback on how the systems reacts on the command the results might be different from the desired behaviour. Examples for the application of open loop control are: A tennis player who hits the ball, if you turn on a light in you room or a DC motor without sensors.

![fig. 1.1: general structure of a open loop control](figures/fig01-001.png)

To improve the resulting behaviour and be able to correct possible differences between the commad value and the resulting value, one can implement sensors that observe the resulting behaviour of the systems or process. This sensors return the current behaviour to the controller and the controller can correct the input signal to the actuator. The following figure shows the general structure of a close loop control.fi

![fig. 1.2: general structure of a close loop control](figures/fig01-002.png)

## Industrial Process

Set of actions for transporting and transforming raw materials to obtain a final product.

## Types of Industrial Processes

### Continouse Processes:

-   Uninterrupted process over time in which analog variables (temperature, speed, etc.) are manipulated. Process in which raw materials are constantly entering at one end of the system, while at the other end a finished product is continuously obtained.

-   Proceso en el que las materias primas están constantemente entrando por un extremo del sistema, mientras que en el otro extremo se obtiene de forma continua un producto terminado.

*Example: Heating System* Someone defines the requiered temperature in the room. The input value can be done by different types of devices, digital or analogue inputs, for example. The at the same time, the current room temperature is measured and compared to the reference. The difference between these two value a the input to the controller and the controller calculates the new opening value for a valve that opens ths gas flow to the burner. The room gets warmer (or not) and a new temparature is measured. This new temparature value is than compared again to the reference value.

![fig. 1.3 Heating System](figures/fig01-003.png)

![fig. 1.4 representation of a heating System in blocks](figures/fig01-004.png)

One can abstact the real system in block diagrams with different levels. In fig.1.4 the complete heating system is represented as one block *Process*. The block has inputs and outputs. in the presented case, the inputs are air, water and gas while the outputs are the air with a desired temperature.

## Industrial Automation
