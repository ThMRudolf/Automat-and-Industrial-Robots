...
Someone defines the requiered temperature in the room. The input value can be done by different types of devices, digital or analogue inputs, for example.
The at the same time, the current room temperature is measured and compared to the reference. The difference between these two value a the input to the controller and the controller calculates the new opening value for a valve that opens ths gas flow to the burner. The room gets warmer (or not) and a new temparature is measured. This new temparature value is than compared again to the reference value.
